using SharpPacker.Strategy.BoxPacker.Models;
using System.Collections.Generic;
using System.Linq;

namespace SharpPacker.Strategy.BoxPacker.Services
{
    internal class WeightRedistributor
    {
        ...
        /// <param name="originalBoxes"></param>
        /// <returns></returns>
        public PackedBoxList RedistributeWeight2(PackedBoxList originalBoxes)
        {
            var targetWeight = originalBoxes.GetMeanWeight();

            var redistrebutedBoxes = originalBoxes.ToList();
            redistrebutedBoxes = redistrebutedBoxes.OrderByDescending(box => box.TotalWeight).ToList();
            ...
            {
                iterationSuccessful = false;
                var previousState = redistrebutedBoxes.Select(box => box.TotalWeight).ToList();
                ...
                while (a - 1 > 0)
                {
                    ...
                    while (b - 1 >= 0)
                    {
                        ...
                        if (iterationSuccessful)
                        {
                            // Xóa bất kỳ hộp nào đã trống ra khỏi danh sách
                            redistrebutedBoxes = redistrebutedBoxes
                                .Where(box => box != null && box.TotalWeight > 0)
                                .OrderByDescending(box => box.TotalWeight)
                                .ToList();
                ...
                // So sánh trạng thái của danh sách trước và sau khi lặp
                var currentState = redistrebutedBoxes.Select(box => box.TotalWeight).ToList();
                if (!currentState.SequenceEqual(previousState))
                {
                    iterationSuccessful = true; // Tiếp tục nếu có thay đổi
                }

            } while (iterationSuccessful);

            // Gộp lại thành một danh sách duy nhất
            var packedBoxes = new PackedBoxList();
            packedBoxes.InsertFromArray(redistrebutedBoxes.ToArray());
