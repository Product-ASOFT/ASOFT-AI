...
namespace SharpPacker.Strategy.BoxPacker.Services
{
    public class VolumePacker
    {
        ...
        private float BoxLength => Math.Min(box.InnerWidth, box.InnerLength);
        private float BoxWidth => Math.Max(box.InnerWidth, box.InnerLength);
        ...
        protected void PackLayer(float startDepth, float widthLeft, float lengthLeft, float depthLeft)
        {
            var newLayer = new PackedLayer();
            layers.Add(newLayer);

            float x, y,rowWidth, rowLength, layerDepth;
            ...
            while (items.Count > 0)
            {
                ...
                {
                    var packedItem = PackedItem.FromOrientatedItem(orientedItem, x, y, (int)startDepth);
        ...
        /// </summary>
        /// <returns></returns>
        private float GetCurrentPackedDepth()
        {
