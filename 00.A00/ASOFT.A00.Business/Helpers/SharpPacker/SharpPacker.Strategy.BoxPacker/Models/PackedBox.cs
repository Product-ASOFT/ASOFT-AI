...
namespace SharpPacker.Strategy.BoxPacker.Models
{
    public class PackedBox : IComparable<PackedBox>
    {
        ...
        public float RemainingDepth => (Box?.InnerDepth ?? 0) - UsedDepth;
        public float RemainingLength => (Box?.InnerLength ?? 0) - UsedLength;
        public float RemainingWidth => (Box?.InnerWidth ?? 0) - UsedWidth;
               
        public float ItemsWeight => ItemsIsEmpty ? 0 : PackedItems.Sum(item => item.Weight);
        public float TotalWeight => (Box?.EmptyWeight ?? 0) + ItemsWeight;
        public float RemainingWeight => (Box?.MaxWeight ?? 0) - TotalWeight;
        ...
        public float UsedDepth => ItemsIsEmpty ? 0 : PackedItems.Max(item => (item.Z + item.Depth));
        public float UsedLength => ItemsIsEmpty ? 0 : PackedItems.Max(item => (item.Y + item.Length));
        public float UsedWidth => ItemsIsEmpty ? 0 : PackedItems.Max(item => (item.X + item.Width));
