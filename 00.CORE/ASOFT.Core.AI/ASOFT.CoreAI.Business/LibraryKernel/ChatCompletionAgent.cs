...
#pragma warning restore SKEXP0110
        ...
        await foreach (var result in invokeResults.ConfigureAwait(false))
        {
            ...
            {
                await NotifyThreadOfNewMessage(chatHistoryAgentThread, result, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return ActivityExtensions.RunWithActivityAsync(
            () => ModelDiagnostics.StartAgentInvocationActivity(Id, agentName, Description),
            () => InternalInvokeAsync(agentName, history, (m) => Task.CompletedTask, arguments, kernel, null, cancellationToken),
            cancellationToken);
    }
...
#pragma warning restore SKEXP0110
    ...
    internal static (IChatCompletionService service, PromptExecutionSettings? executionSettings) GetChatCompletionService(Kernel kernel, KernelArguments? arguments)
    {
        ...
        kernel.ServiceSelector.TrySelectAIService(kernel, nullPrompt, arguments ?? [], out IChatCompletionService? chatCompletionService, out PromptExecutionSettings? executionSettings);
...
#pragma warning restore CA2000
        ...
        CancellationToken cancellationToken)
    {
        ...
        string? instructions = await RenderInstructionsAsync(kernel, arguments, cancellationToken).ConfigureAwait(false);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        ChatHistory chat = await SetupAgentChatHistoryAsync(history, arguments, kernel, additionalInstructions, cancellationToken).ConfigureAwait(false);
