...
public sealed class ChatHistoryChannel : AgentChannel
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await historyAgent.ReduceAsync(_history, cancellationToken).ConfigureAwait(false);
...
#pragma warning disable CS0618 // Type or member is obsolete
        await foreach (ChatMessageContent responseMessage in historyAgent.InvokeAsync(_history, null, null, cancellationToken).ConfigureAwait(false))
#pragma warning restore CS0618 // Type or member is obsolete
        {
            // Capture all messages that have been included in the mutated the history.
            for (int messageIndex = messageCount; messageIndex < _history.Count; messageIndex++)
            {
                ChatMessageContent mutatedMessage = _history[messageIndex];
...
#pragma warning disable CS0618 // Type or member is obsolete
        await foreach (StreamingChatMessageContent streamingMessage in historyAgent.InvokeStreamingAsync(_history, null, null, cancellationToken).ConfigureAwait(false))
#pragma warning restore CS0618 // Type or member is obsolete
        {
            yield return streamingMessage;
        }

        for (int index = messageCount; index < _history.Count; ++index)
        {
    ...
    {
        return _history.ToAsyncEnumerable();
    }
